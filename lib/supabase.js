...
export async function getGameLeaderboard(gameId, limit = 1000) {
...
export async function getCentralLeaderboard(limit = 1000) {
  // Sum best_score across all games per user, join with username
  const { data, error } = await supabase
    .from('users')
