export class AISystem {
  ...
  _findBestFlightTime(dx, dy, gravity) {
    const candidates = [0.45, 0.55, 0.65, 0.75, 0.85, 0.95, 1.05]
    let bestT     = 0.65
    ...
    for (const t of candidates) {
      ...
      const arcScore = vy > 0 ? (1 / (1 + Math.abs(speed - 14))) : -10
  ...
  // AI repositions aggressively — jumps around the court
  getRepositionTarget(THREE) {
    const accuracy = this._cfg.aiAccuracy || 0.5

    // Randomise position across the right half — AI flies around
    const optimalDist = 1.5 + accuracy * 5.0
    const spread      = 3.0   // wide spread so AI moves a lot
