export class AISystem {
  ...
  requestShot(CANNON, onShot) {
    ...
    // Very fast reaction — hard from level 1
    const base   = Math.max(0.08, (this._cfg.aiReactionTime || 0.8) * 0.25)
    const jitter = (Math.random() - 0.5) * 0.08
    this._decisionTimer = Math.max(0.05, base + jitter)
  ...
  _calcShotToHoop(CANNON) {
    ...
    // Always use full gravity — AI knows physics perfectly
    const gravity = this._cfg.gravity || 15
    // Near-perfect accuracy from level 1
    const accuracy = Math.min(0.98, 0.85 + (this._cfg.level || 1) * 0.005)
    ...
    // Very small error — AI is very accurate
    const error = Math.max(0.02, (1 - accuracy) * 0.8)
