...
export class TurnManager {
  ...
  _bindInput(side) {
    ...
    // ── POINTER DOWN: start drag — snap player to ball if ball is ready ──
    this.input.onPointerDown((worldPos) => {
      ...
      // Allow dragging when:
      //  a) ball is in hand (attached), OR
      //  b) ball is flying but has nearly stopped (speed < 2 u/s) — pick it up
      const speedSq = ((ball._flyVx || 0) ** 2) + ((ball._flyVy || 0) ** 2)
      const ballReady = ball.state === 'attached' ||
                        (ball.state === 'flying' && speedSq < 4)

      if (!ballReady) return   // ball is still actively flying — don't interrupt

      const bx = ball.mesh.position.x
      const by = Math.max(0, ball.mesh.position.y)
      player.position.set(
        Math.max(-11.5, Math.min(11.5, bx)),
        Math.max(0, by),
        0
      )
      player._sync()
    ...
    this.input.onPointerUp((worldPos) => {
      ...
      // Speed scales with drag distance — taller court needs more power
      const speed = Math.max(6, Math.min(32, len * 4.5))
    ...
    // ── TAP: snap player to ball instantly (pointerDown already did this,
    //    but keep onTap for safety in case drag never fired) ──
    this.input.onTap((worldPos) => {
      // Nothing extra needed — pointerDown already snapped player to ball.
      // Just make sure state is idle so next drag works.
      if (!this._drag[side]) {
  ...
  _tickSide(side, delta) {
    ...
    if (state === 'shot_in_flight' && ball.state === 'attached') {
      ...
      if (!this._isPVP && side === 'right') {
        ...
        player._onTeleportComplete = () => {
          ...
          this._triggerAIShot()   // shoot immediately after landing
        }
      }
      return
    }
    ...
    // Force AI shot if it's been idle too long — AI never waits more than ~2s
    if (!this._isPVP && side === 'right' && state === 'idle') {
      if (this.ai.shouldForceShot()) {
        this._triggerAIShot()
      }
    }
  }
