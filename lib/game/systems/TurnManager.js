...
export class TurnManager {
  ...
  _bindInput(side) {
    ...
    this.input.onPointerDown((worldPos) => {
      ...
      // Start drag if touching near the player (generous hit zone)
    ...
    // POINTER MOVE — update straight arrow
    this.input.onPointerMove((worldPos) => {
      if (!this._drag[side]) return
      // Arrow: start = player position, end = drag position
      // Shows direction and power
      this.dragArrow.update(this._dragStart[side], worldPos)
    })

    // POINTER UP — shoot ball STRAIGHT in drag direction
    this.input.onPointerUp((worldPos) => {
      ...
      // Only shoot if ball is attached to this player
      if (ball.state !== 'attached') return

      const start = this._dragStart[side]
      // Direction: from start toward worldPos (drag direction)
      ...
      // Speed scales with drag length — straight shot, no arc
      const speed = Math.max(GAME_CONFIG.DRAG_MIN_POWER, Math.min(GAME_CONFIG.DRAG_MAX_POWER, len * 2.5))
