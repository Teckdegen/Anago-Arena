...
export class TurnManager {
  ...
  _bindInput(side) {
    ...
    // ── POINTER DOWN: start drag from ANYWHERE — teleport to ball if needed ──
    this.input.onPointerDown((worldPos) => {
      ...
      if (ball.state === 'attached') {
        // If player is far from ball, teleport them to it instantly before dragging
        const bx = ball.mesh.position.x
        const by = Math.max(0, ball.mesh.position.y)
        const distToBall = Math.abs(player.position.x - bx) + Math.abs(player.position.y - by)
        if (distToBall > 0.5) {
          // Snap player to ball position immediately (no animation delay)
          player.position.set(
    ...
    this.input.onPointerUp((worldPos) => {
      ...
      // Speed scales with drag distance — short drag = slow, long drag = fast
      // Min 4, max 20 — much more controlled than before
      const speed = Math.max(4, Math.min(20, len * 3.5))
