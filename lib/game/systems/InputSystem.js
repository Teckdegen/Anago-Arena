export class InputSystem {
  constructor(canvas, camera, THREE) {
    ...
    this._hasMoved        = false   // any movement at all since down
  ...
  screenToWorld(screenX, screenY, z = 0) {
    ...
    if (this.camera.isOrthographicCamera) {
      // For ortho, unproject gives the exact world XY — just set Z
      vec.z = z
      return vec
    }

    // Perspective: ray-plane intersection at z=0 plane
  ...
  _pointerMove(screen) {
    ...
    // Mark as dragging after just 2px — instant response
    if (dist > 2) {
    ...
    const world = this.screenToWorld(screen.x, screen.y)
    this._cbPointerMove.forEach(cb => cb(world, screen))
  }
  ...
  _pointerUp(screen) {
    ...
    // ALWAYS fire pointerUp — every release counts no matter how fast
    this._cbPointerUp.forEach(cb => cb(world, screen, this._isDragging))

    // Tap = tiny movement (under 8px) AND quick (under 250ms)
    if (dragDist < 8 && elapsed < 250) {
