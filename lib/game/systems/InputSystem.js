export class InputSystem {
  ...
  _pointerMove(screen) {
    ...
    // Mark as dragging after just 4px — very responsive
    if (dist > 4) {
  ...
  _pointerUp(screen) {
    ...
    // Fire pointerUp for ALL releases — drag or not
    this._cbPointerUp.forEach(cb => cb(world, screen, this._isDragging))

    // Tap = very small movement AND quick (under 300ms)
    // Even a 2-second drag should NOT fire tap
    if (dragDist < 10 && elapsed < 300 && !this._hasMoved) {
