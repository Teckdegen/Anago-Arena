...
export class Hoop {
  ...
  _build() {
    ...
    const rimTube = Math.max(0.09, r * 0.055)
  ...
  checkScore(ballMesh, prevBallY) {
    ...
    // X-tolerance scales with ball radius so scoring works at any size
    const inX         = Math.abs(bx - hx) < (GAME_CONFIG.BALL_RADIUS * 1.1)
