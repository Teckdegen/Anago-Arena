...
export class Ball {
  ...
  launch(velocity, levelConfig) {
    ...
    this.body.type = this.CANNON.Body.KINEMATIC  // kinematic = not affected by gravity
    ...
    this._flyTimer = 0
    // Store velocity for manual update (kinematic bodies need manual position update)
    this._flyVx = velocity.x
    this._flyVy = velocity.y
    this._flyVz = velocity.z || 0
  }

  // Alias — same as launch, always straight
  ...
  update(delta) {
    ...
    if (this.state === 'attached') {
      ...
      // Kinematic straight-line: manually advance position each frame
      // Add very gentle downward drift so ball eventually comes down
      this._flyVy = (this._flyVy || 0) - 0.8 * delta   // slow gravity on ball
      ...
      if (this.body.position.y < 0) {
        ...
        this._flyVy = Math.abs(this._flyVy) * 0.4   // weak bounce
        this._flyVx *= 0.85
      }
