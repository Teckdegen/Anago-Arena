...
export class Ball {
  ...
  _buildMesh() {
    ...
    // Black seam lines — tube thickness proportional to ball size
    const seamGeo = new T.TorusGeometry(GAME_CONFIG.BALL_RADIUS + 0.01, GAME_CONFIG.BALL_RADIUS * 0.035, 6, 24)
  ...
  _buildTrail() {
    ...
    // Trail spheres scale with ball size so they always look proportional
    const radii    = [R*0.36, R*0.30, R*0.24, R*0.19, R*0.15, R*0.11, R*0.08, R*0.05]
    const opacities = [0.65,   0.50,   0.38,   0.27,   0.18,   0.12,   0.08,   0.04]
  ...
  update(delta) {
    ...
    if (this.state === 'attached') {
      ...
      if (this.body.position.y < GAME_CONFIG.BALL_RADIUS) {
        this.body.position.y = GAME_CONFIG.BALL_RADIUS
