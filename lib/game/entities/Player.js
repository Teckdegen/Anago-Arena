  ...
  update(delta) {
    ...
    if (this.state === 'repositioning' && this._arcFrom && this._arcTo) {
      ...
      if (this._arcProgress >= 1) {
        ...
        // Custom easing: fast rise (first 30%), slow float down (last 70%)
        // y = sin curve but with asymmetric timing
        let yFactor
        if (t < 0.3) {
          // Fast launch up — ease out
          yFactor = Math.sin((t / 0.3) * Math.PI * 0.5)
        } else {
          // Slow float down — ease in very gently
          const tDown = (t - 0.3) / 0.7
          yFactor = Math.cos(tDown * Math.PI * 0.5)
        }

        const dist = this._arcFrom.distanceTo(this._arcTo)
        const arcH = 1.2 + dist * 0.18   // higher arc

        // X moves with smooth ease
