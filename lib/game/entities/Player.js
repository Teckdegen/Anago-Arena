...
export class Player {
  constructor(scene, THREE, side, color, username) {
    ...
    this.stunned = false
    this.stunTimer = 0
    this.stunImmunity = 0   // seconds of immunity after stun
    this.hasBall = true
    ...
    // Scale relative to court — bigger so visible on tall screens
    this.group.scale.set(3.0, 3.0, 3.0)
  ...
  // New: instant teleport to any position — super fast arc
  jumpTo(targetPos) {
    this.teleportTo(targetPos)
  }

  // Teleport at super speed — very short arc duration
  teleportTo(targetPos) {
    this._arcFrom    = this.position.clone()
    this._arcTo      = new this.THREE.Vector3(
      Math.max(-11.5, Math.min(11.5, targetPos.x)),
      Math.max(0, targetPos.y || 0),
      0
    )
    this._arcProgress = 0
    this._arcCallback = null
    this._teleporting = true   // flag for super-speed
    this.state = 'repositioning'
  }

  // Super-speed dash — moves at very high speed toward target X
  // Player still has gravity (falls down) but moves fast horizontally
  dashTo(targetPos) {
    const tx = Math.max(-11.5, Math.min(11.5, targetPos.x))
    this._dashTargetX = tx
    this._dashing = true
    this.state = 'repositioning'
  }
  ...
  stun() {
    ...
    this.stunImmunity = 10.0   // 10 seconds immunity after stun ends
    this.state = 'stunned'
  ...
  update(delta) {
    ...
    // ── DASH — super-speed horizontal movement with gravity ──
    if (this._dashing && this._dashTargetX !== undefined) {
      ...
      if (dist < 0.15) {
        ...
        // Move at super speed (18 units/sec)
        const step = Math.min(18 * delta, dist)
        this.position.x += Math.sign(dx) * step
      }

      // Gravity — always fall down
    ...
    // Arc repositioning — fast up, slow float down
    if (this.state === 'repositioning' && this._arcFrom && this._arcTo) {
      // Super speed when teleporting (tap), normal arc otherwise
      ...
      if (this._arcProgress >= 1) {
        ...
        this.group.rotation.z = 0
        if (cb) cb()
        if (onTeleport) onTeleport()  // fires ball.attachTo after teleport
        // After landing, gravity pulls down
      } else {
        const t = this._arcProgress

        let yFactor
        if (this._teleporting) {
          // Teleport: go directly, minimal arc
          yFactor = Math.sin(Math.PI * t) * 0.3
        } else {
          // Normal jump: fast rise, slow float down
          if (t < 0.3) {
            yFactor = Math.sin((t / 0.3) * Math.PI * 0.5)
          } else {
            const tDown = (t - 0.3) / 0.7
            yFactor = Math.cos(tDown * Math.PI * 0.5)
          }
        }
      ...
      // Gravity — player falls down when in air
      if (this.position.y > 0) {
        this.position.y = Math.max(0, this.position.y - delta * 3.5)
        this._sync()
      }
