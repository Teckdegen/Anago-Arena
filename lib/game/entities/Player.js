...
export class Player {
  ...
  moveTo(targetPos, onComplete) {
    ...
    this._arcTo      = new this.THREE.Vector3(
      Math.max(-7.5, Math.min(7.5, targetPos.x)), // full court, no half restriction
      0, 0
    )
    this._arcProgress = 0
  ...
  // New: jump to any position — fast up, slow float down, no callback needed
  jumpTo(targetPos) {
    this._arcFrom    = this.position.clone()
    this._arcTo      = new this.THREE.Vector3(
  ...
  update(delta) {
    ...
    if (this.state === 'repositioning' && this._arcFrom && this._arcTo) {
      ...
      if (this._arcProgress >= 1) {
        ...
        if (cb) cb()
        // After landing, go to idle (state set by TurnManager)
      } else {
        const t = this._arcProgress

        // Fast rise (first 30%), slow float down (last 70%)
        let yFactor
        if (t < 0.3) {
        ...
        this._sync()
        this.group.rotation.z = Math.sin(Math.PI * t) * (this.side === 'left' ? -0.32 : 0.32)
      }
    } else if (this.state !== 'repositioning') {
      // Gravity — if player is in the air (y > 0) and not repositioning, fall slowly
      if (this.position.y > 0) {
        this.position.y = Math.max(0, this.position.y - delta * 2.5)
        this._sync()
      }
      this.group.rotation.z = 0
    }
