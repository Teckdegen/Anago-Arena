...
export class Player {
  constructor(scene, THREE, side, color, username) {
    ...
    // Ball attach point — raised right hand (group-local; group.scale=5 → world ×5)
    // y=1.10 → world y ~5.5 units (near top of a 7.5-unit player = extended arm up)
    this.rightHand = new THREE.Object3D()
    this.rightHand.position.set(0.28, 1.10, 0.18)
    ...
    // Scale 7.0 → model height 1.5 × 7 = 10.5 world units (big, visible players)
    this.group.scale.set(side === 'right' ? -7.0 : 7.0, 7.0, 7.0)
  ...
  _onModelLoaded(model) {
    ...
    // Hand attach point — raised arm (group-local); 0.78 of model height = upper arm/hand area
    this.rightHand.position.set(size.x * scale * 0.32, size.y * scale * 0.78, 0.18)
  }
  ...
  _buildFallback() {
    ...
    this.rightHand.position.set(0.28, 0.90, 0.18)
  }
  ...
  _buildLabel() {
    ...
    this.label.position.set(0, 1.75, 0)  // group-local; ×7 world = 12.25 units above floor
  ...
  update(delta) {
    ...
    if (this.stunned) {
      ...
      this.stunStars.forEach((star, i) => {
        ...
        // group-local coords (group.scale=7 → world ×7); 1.6 = top of model head
        star.position.set(Math.cos(a) * 0.42, 1.6, Math.sin(a) * 0.42)
      })
    }
    ...
    if (this._dashing && this._dashTargetX !== undefined) {
      ...
      if (this.position.y > 0)
        this.position.y = Math.max(0, this.position.y - delta * 5.25)
      this._sync()
    ...
    if (this.state === 'holding' || this.state === 'waiting') {
      ...
      if (this.label)  this.label.position.y = 1.75 + bob
