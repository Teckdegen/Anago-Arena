...
export class Court {
  ...
  // courtH is passed in from GameCanvas (computed from screen size)
  rebuild(levelConfig, courtH) {
    this._dispose()
    const T = this.THREE
    const C = this.CANNON
    const theme = levelConfig.courtTheme
    const wallH = levelConfig.wallHeight || 5

    const hw = GAME_CONFIG.COURT_HALF_WIDTH          // 12
    const ch = courtH || GAME_CONFIG.COURT_HEIGHT || 32  // dynamic screen height

    // ── Floor ────────────────────────────────────────
    const floorGeo = new T.BoxGeometry(hw * 2, 0.5, 3)
    ...
    floorBody.addShape(new C.Box(new C.Vec3(hw, 0.25, 1.5)))
    floorBody.position.set(0, -0.25, 0)
    this.world.addBody(floorBody)
    this._bodies.push(floorBody)

    // ── Court centre line ─────────────────────────────
    const lineGeo = new T.BoxGeometry(0.08, 0.02, 3)
    ...
    const topEdge = new T.Mesh(new T.BoxGeometry(hw * 2, 0.12, 0.12), edgeMat)
    topEdge.position.set(0, ch, 0)
    this.scene.add(topEdge); this._meshes.push(topEdge)

    const floorEdge = new T.Mesh(new T.BoxGeometry(hw * 2, 0.08, 0.08), edgeMat)
