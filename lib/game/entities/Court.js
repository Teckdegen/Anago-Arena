...
export class Court {
  ...
  rebuild(levelConfig) {
    ...
    const courtH = GAME_CONFIG.COURT_HEIGHT        // 32
    ...
    const leftWallGeo = new T.BoxGeometry(0.4, courtH, 2.5)
    ...
    lBound.addShape(new C.Box(new C.Vec3(0.25, courtH / 2 + 1, 2)))
    lBound.position.set(-hw - 0.2, courtH / 2, 0)
    this.world.addBody(lBound)
    this._bodies.push(lBound)

    // ── Right side wall — visible + physics ──────────
    const rightWallGeo = new T.BoxGeometry(0.4, courtH, 2.5)
    ...
    rBound.addShape(new C.Box(new C.Vec3(0.25, courtH / 2 + 1, 2)))
    rBound.position.set(hw + 0.2, courtH / 2, 0)
    ...
    ceiling.position.set(0, courtH + 0.25, 0)
    this.world.addBody(ceiling)
    this._bodies.push(ceiling)

    // ── Background plane ──────────────────────────────
    const bgGeo = new T.PlaneGeometry(hw * 2 + 6, courtH + 6)
