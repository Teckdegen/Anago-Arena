export const GAME_CONFIG = {
  ...
  WIN_SCORE: 100,
  STUN_DURATION: 2.0,
  BALL_RADIUS: 0.62,        // hand-sized relative to player (group.scale=5 → world radius ~3.1)
  COURT_HEIGHT: 32,         // tall court — fills portrait screens
  ARC_DURATION: 1.1,
  BALL_RETURN_SPEED: 9,
  DRAG_MAX_POWER: 14,
  DRAG_MIN_POWER: 3,
...
export function getLevelConfig(level) {
  ...
  return {
    level: n,
    // Hoop shrinks every 10 levels — only linear change
    hoopRadius:      Math.max(1.2, 2.6 - Math.floor(n / 10) * 0.08),
    // Physics changes with level
    gravity:         15 + n * 0.09,
    ballRestitution: 0.48 + r1 * 0.32,
    ballFriction:    0.1 + r2 * 0.15,
    // Court layout — wall is taller to match new court height
    wallHeight:      5.0 + r2 * 2.5,
