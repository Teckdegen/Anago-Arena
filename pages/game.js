...
const BasketballCanvas = dynamic(() => import('../components/games/BasketballCanvas'), { ssr: false })
...
export default function BasketballPage() {
    ...
    window.ANAGO_UI = {
      ...
      showResult: async (winner, finalScores) => {
        ...
        if (s) {
          ...
          await upsertGameScore(u.wallet, 'basketball', { score: finalScores[0], won: winner === 0 })
          if (winner === 0) {
            const old = parseInt(localStorage.getItem('bb_points') || '0')
            localStorage.setItem('bb_points', String(old + finalScores[0] * 20))
          }
        }
      },
    }
  ...
  if (gameState === 'playing') return (
    <>
      ...
      <div className="fixed top-0 left-0 right-0 z-10 flex justify-between items-center p-3 pointer-events-none">
        <div className="hud-panel">
          <p className="hud-score" style={{ color: '#C17A2A', display: 'flex', alignItems: 'center', gap: 4 }}>
            <PawIcon size={11} color="#C17A2A" /> {user?.username || 'YOU'}
          </p>
          <p className="hud-score" style={{ color: '#F5EFE0' }}>{scores[0]}</p>
        </div>
        <div className="hud-panel text-center">
          <p className="hud-score" style={{ fontSize: 7, color: '#F4A0A0' }}>BASKETBALL</p>
        </div>
      ...
      <BasketballCanvas config={gameConfig} />
    </>
  )

  return (
    <div className="fixed inset-0 z-30 flex items-center justify-center p-4" style={{ background: 'rgba(15,8,40,0.92)' }}>
      <div className="cartoon-card text-center" style={{ maxWidth: 340, padding: '32px 24px', borderColor: result.winner === 0 ? '#C17A2A' : '#E05050' }}>
        ...
        <div className="flex flex-col gap-3 mt-6">
          <button className="btn-arcade w-full" style={{ fontSize: 10, background: '#C17A2A' }}
