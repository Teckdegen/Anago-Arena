...
const DashCanvas = dynamic(() => import('../../components/games/DashCanvas'), { ssr: false })
...
export default function DashPage() {
    ...
    window.ANAGO_UI = {
      ...
      showResult: async (winner, finalScores) => {
        ...
        if (saved) {
          ...
          await upsertGameScore(u.wallet, 'dash', { score: finalScores[0], won: winner === 0 })
  ...
  if (gameState === 'lobby') return (
    <>
      <Head><title>Dog Dash – ANAGO ARENA</title></Head>
      <GameLobby gameId="dash" gameName="Dog Dash" gameEmoji="🐾" accentColor="#E05050"
  ...
  if (gameState === 'playing') return (
    <>
      ...
      <div className="fixed top-0 left-0 right-0 z-10 flex justify-between items-center p-3 pointer-events-none">
        <div className="hud-panel"><p className="hud-score" style={{ color: '#E05050' }}>{user?.username || 'YOU'}: {scores[0]}</p></div>
        <div className="hud-panel"><p className="hud-score" style={{ fontSize: 8, color: '#F4A0A0' }}>DOG DASH</p></div>
    ...
    <div className="fixed inset-0 z-30 flex items-center justify-center p-4" style={{ background: 'rgba(15,8,40,0.92)' }}>
      <div className="cartoon-card text-center" style={{ maxWidth: 340, padding: '32px 24px' }}>
        ...
        <div className="flex flex-col gap-3 mt-6">
          <button className="btn-arcade w-full" style={{ fontSize: 10, background: '#E05050' }}
